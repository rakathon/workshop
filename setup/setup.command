...
SNOWFLAKE_CONFIG="$HOME/snowflake-mcp/tools_config.yaml"
SNOWFLAKE_SED=$(printf '%s' "$SNOWFLAKE_CONFIG" | sed 's/[&/\]/\\&/g')
...
printf '%s' "$MOBILECONFIG_FALLBACK" \
  ...
  | sed "s|{{SNOWFLAKE_CONFIG}}|${SNOWFLAKE_SED}|g" \
  > "$MOBILECONFIG_TMP"
