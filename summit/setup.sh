...
if [[ "$PREV_INSTALL" == "Yes — Update Project" ]]; then
  ...
  if [[ -d "${DESKTOP}/dishly-platform" ]] && command -v node &>/dev/null; then
    ...
    run "Starting dishly-platform (deps, migrate, seed, servers)..."
    ( cd "${DESKTOP}/dishly-platform" && bash start.sh > /dev/null 2>&1 & ) || true
    sleep 5
...
if command -v claude &>/dev/null; then
  for plugin in forge forge-product-management forge-skill-creator; do
    ...
    claude plugin remove "${plugin}@rr-standards" 2>&1 || true
    run "Installing ${plugin}..."
    if claude plugin install "${plugin}@rr-standards" 2>&1; then
      ok "${plugin} installed"
    else
